...
test("el panel limita la exportación a PDF, imagen y DOCX editable", async () => {
  const html = await readFile("index.html", "utf8");
  for (const label of ["Exportar PDF", "Exportar imagen", "Exportar DOCX editable", "Abrir tablero editable", "Imprimir tablero"]) {
    assert.match(html, new RegExp(label));
  }
  assert.doesNotMatch(html, /Exportar HTML|Exportar JPG|Exportar PNG|Guardar copia editable/);
});
