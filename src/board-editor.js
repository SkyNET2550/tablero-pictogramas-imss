...
function addBlankPage(sourceBoardId = activeId) {
  const board = boards.find(item => item.id === sourceBoardId) || activeBoard();
...
function makeBoardPage(board, pageIndex = 0, totalPages = 1) {
  ...
  controls.innerHTML = `<span>Página ${pageIndex + 1} de ${totalPages}</span><button type="button" data-page-action="validate">Validar página</button><button type="button" data-page-action="delete">Eliminar página</button><button type="button" data-page-action="add">Agregar página</button>`;
...
function makeCell(cell, index, boardId = activeId) {
  ...
  article.querySelector("strong").addEventListener("click", event => {
    ...
    editCellLabel(article, cell);
  });
  const label = article.querySelector("strong");
  label.classList.add("cell-label");
  label.tabIndex = 0;
  label.setAttribute("role", "button");
  label.setAttribute("aria-label", `Editar nombre: ${cell.label}`);
  ...
  label.addEventListener("pointerup", startLabelEdit);
  label.addEventListener("dblclick", startLabelEdit);
  label.addEventListener("keydown", event => {
    if (event.key === "Enter" || event.key === " ") startLabelEdit(event);
  });
  article.addEventListener("click", event => {
    if (event.target.closest("strong")) {
      startLabelEdit(event);
      return;
    }
    document.querySelectorAll(".editor-cell.selected").forEach(item => item.classList.remove("selected"));
    article.classList.add("selected");
...
function makeWordCompatibleHtml(board) {
  const cells = printableCells(board).map(cell => cell ? `<td><div class="picto-box"><img src="${escapeHtml(cell.imageData || cell.imageUrl || getPictogramImageUrl(cell.id))}" width="120" height="100" style="width:1.25in;height:1.04in;"><p>${escapeHtml(cell.label)}</p></div></td>` : "<td></td>");
  const rows = [0, 4, 8, 12].map(start => `<tr>${cells.slice(start, start + 4).join("")}</tr>`).join("");
  return `<!doctype html><html><head><meta charset="utf-8"><style>@page{size:letter portrait;margin:.35in}body{font-family:Arial,sans-serif;color:#082f61}.brand{width:4.65in;height:.72in}h1{text-align:center;text-transform:uppercase;border-bottom:3px solid #0757a5;padding-bottom:8px;margin:4pt 0 8pt;font-size:24pt;line-height:1}table{width:7.25in;table-layout:fixed;border-collapse:separate;border-spacing:5pt}tr{height:1.45in}td{width:1.72in;height:1.45in;border:2pt solid #0757a5;border-radius:8pt;text-align:center;vertical-align:middle;padding:4pt;overflow:hidden}.picto-box{width:1.55in;height:1.34in;margin:0 auto;text-align:center;overflow:hidden}td img{display:block;width:1.25in;height:1.04in;margin:0 auto;border:0}td p{height:.25in;margin:2pt 0 0;color:#111;font-weight:bold;font-size:12pt;line-height:1.05;text-align:center;overflow:hidden}footer{font-size:5pt;text-align:center;color:#555}</style></head><body><img class="brand" src="${BOARD_HEADER_IMAGE}" width="446" height="69"><p style="text-align:center;font-weight:bold;color:#004b93;margin:2pt 0">TABLERO DE COMUNICACIÓN POR PICTOGRAMAS</p><h1>${escapeHtml(board.title)}</h1><table>${rows}</table><footer>${escapeHtml(INSTITUTIONAL_FOOTER)}</footer></body></html>`;
}
